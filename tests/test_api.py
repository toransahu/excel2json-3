...
try:
    from convert import download_file, save_file, read_excel_sheet, jsonify
except ImportError:
    from api.convert import download_file, save_file, read_excel_sheet, jsonify

URL = (
    "https://www.iso20022.org/sites/default/files/ISO10383_MIC/ISO10383_MIC.xls"
)  # Correct
URL1 = (
    "https://www.iso20022.org/sites/default/files/ISO10383_MIC/ISO10383_MI.xls"
)  # C is missing before .xls
LOC = "../data/"
LOC1 = ""
LOC2 = "abc"
...
def test_read_invalid_excel_sheet():
    assert read_excel_sheet(os.path.join(LOC, INVALID_FILE), SHEET_NAME) is None


# passed invalid sheet name
def test_read_invalid_excel_sheet_name():
    assert read_excel_sheet(os.path.join(LOC, FILENAME), SHEET_NAME_INCORRECT) is None


# passed invalid directory path
def test_read_excel_sheet_from_invalid_dir():
    assert read_excel_sheet(os.path.join(LOC2, FILENAME), SHEET_NAME) is None


# passed empty worksheet
def test_jsonify_empty_ws():
    assert jsonify(None, LOC) == 1


# passed invalid path
def test_jsonify_invalid_path():
    assert jsonify(read_excel_sheet(os.path.join(LOC, FILENAME), SHEET_NAME), LOC2) == 1
